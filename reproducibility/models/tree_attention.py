...
from reproducibility.modules import SingleTimeDistributed
...
class TreeAttention(Model):
    ...
    def _get_node_probabilities(self, embedded_nodes, embedded_premise, nodes_mask, premise_mask,
        ...
        expanded_nodes_mask_premise = nodes_mask.unsqueeze(-1).unsqueeze(-1).expand_as(
            normalized_node_premise_attention).float()
        # aggregate representation. dim: batch x nodes x premise words
        mean_node_premise_attention = masked_mean(normalized_node_premise_attention,
                                                  expanded_nodes_mask_premise, dim=2)


        # convert batch x nodes and batch x premise to batch x nodes x premise mask
        nodes_only_mask = (torch.sum(nodes_mask, -1) > 0).float()
        node_premise_mask = nodes_only_mask.unsqueeze(-1).unsqueeze(-1).expand_as(mean_node_premise_attention) \
        ...
        expanded_nodes_mask_embedding = nodes_mask.unsqueeze(-1).unsqueeze(-1).expand_as(
            embedded_nodes).float()
        # dim: batch x nodes x emb. dim
        aggregate_node_representation = masked_mean(embedded_nodes, 
                                                    expanded_nodes_mask_embedding, dim=2)
        ...
        nodes_class_mask = nodes_only_mask.unsqueeze(-1).unsqueeze(-1).expand_as(
            phrase_prob_distribution).float()

        mean_phrase_distribution = masked_mean(phrase_prob_distribution,  nodes_class_mask, dim=1)
    ...
    def _get_edge_probabilities(self, encoded_premise, mean_node_premise_attention, edge_sources,
        ...
        masked_edge_labels = replace_masked_values(edge_labels.float(), edge_mask.squeeze(-1), 0).long()
        ...
        mean_edge_distribution = masked_mean(edge_prob_distribution, edges_only_mask, 1)
