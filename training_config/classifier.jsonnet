...
local SEQ2VEC_FIELDS(embedding_dim, averaged) = {
    "type": "seq2vec",
    "architecture": {
...
local TOKEN_INDEXERS = if std.extVar("EMBEDDING") == "ELMO_LSTM" then ELMO_CHAR_LSTM_FIELDS['elmo_lstm_indexer'] else {}
...
local TOKEN_EMBEDDERS = if std.extVar("EMBEDDING") == "ELMO_LSTM" then ELMO_CHAR_LSTM_FIELDS['elmo_lstm_embedder'] else {}
...
local EMBEDDING_DIM = if std.extVar("EMBEDDING") == "ELMO_LSTM" then ELMO_CHAR_LSTM_FIELDS['embedding_dim'] else 0
...
local ENCODER = if std.extVar("ENCODER") == "AVERAGE" then SEQ2VEC_FIELDS(EMBEDDING_DIM, true) else {} + 
                if std.extVar("ENCODER") == "SUM" then SEQ2VEC_FIELDS(EMBEDDING_DIM, false) else {} + 
